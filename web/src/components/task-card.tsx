...
export function TaskCard({ id, creator, prompt, bounty, minAgents, maxAgents, deadline, status, claimedCount = 0 }: TaskCardProps) {
  return (
    <Link href={`/tasks/${id}`} className="block p-5 rounded-[24px] bg-[var(--color-surface-card)] border border-[var(--color-border-card)] hover:bg-[var(--color-surface-hover)] transition-all group">
      ...
      <p className="text-sm text-[var(--color-bone)] line-clamp-2 mb-3 leading-relaxed group-hover:text-[var(--color-plum-voltage)] transition-colors">{prompt}</p>
